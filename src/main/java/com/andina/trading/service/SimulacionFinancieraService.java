 ...
 * Permite obtener datos históricos de precios y proyectar valores futuros mediante
 * técnicas de análisis simple, como la media móvil.
 *
...
public class SimulacionFinancieraService {
    ...
    /**
     * Obtiene los precios históricos de una acción específica usando la API de Yahoo Finance.
     * <p>Realiza una solicitud HTTP a la API de Yahoo Finance para obtener los precios de mercado
     * de una acción determinada y los almacena en una lista para su análisis.</p>
     *
    ...
    /**
     * Realiza una simulación de proyección del precio futuro de una acción usando una técnica de media móvil simple.
     * <p>Calcula el precio proyectado como el promedio de los precios históricos proporcionados.</p>
     *
     * @param preciosHistoricos lista de precios históricos de la acción
     * @return el precio futuro proyectado, o {@code BigDecimal.ZERO} si no hay datos históricos
     */
    public BigDecimal proyectarPrecioFuturo(List<BigDecimal> preciosHistoricos) {
        ...
        return preciosHistoricos.isEmpty() ? BigDecimal.ZERO : total.divide(BigDecimal.valueOf(preciosHistoricos.size()), BigDecimal.ROUND_HALF_UP);
    }
}
